...
mod tests {
    ...
    fn should_generate_builder_for_struct_with_one_property() {
        ...
        let gleipnir = Gleipnir::builder()
            .roots_of("mountains".to_string())
            .maybe_roots_of(Some("mountains".to_string()))
            .build();
    ...
    #[test]
    fn should_generate_builder_for_tuple_struct() {
        #[derive(Builder)]
        struct Gleipnir(String, u8, Vec<String>);

        let gleipnir = Gleipnir::builder()
            .field_0("mountains".to_string())
            .field_1(1)
            .field_2(vec![
                "sound of cat's footsteps".to_string(),
                "beard of a woman".to_string(),
        ...
        assert_eq!(gleipnir.0, "mountains".to_string());
        assert_eq!(gleipnir.1, 1);
        assert_eq!(gleipnir.2.len(), 3)
    }
